...
# TP1 = not sig (p=0.1055), KWH=4.4988, df=2
# TP2 = not sig (p=0.1013), KWH=4.5797, df=2
# TP3 = not sig (p=0.4073), KWH=1.7966, df=2
...
#
#
exp_group <- unique(ratios.non.na.values$exp_group) # List of groups
...
for (exp_group in exp_group) {
  ...
  # Subset data for the current timepoint
  subset.data2 <- ratios.non.na.values[ratios.non.na.values$exp_group == exp_group, ]
...
# TPs not sig different in TBSV p=0.7984, KWH = 0.45036, df=2
# TPs not sig diff in TBSV_DI72 p=0.9307, KWH = 0.14353, df=2
# TPs not sig diff in TBSV_DI73 p=0.5906, KWH = 1.0532, df=2
